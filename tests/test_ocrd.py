...
from ocrd_doxa.doxa_binarize import DoxaBinarize

def test_image(processor_kwargs, subtests, caplog):
    caplog.set_level(logging.INFO)
    def only_doxa(logrec):
        return logrec.name == 'ocrd.processor.DoxaBinarize'
    ...
    for algo in ["Sauvola"]:
        ...
        with subtests.test(
                msg="on image input",
                algorithm=algo):
            with caplog.filtering(only_doxa):
                run_processor(
                    DoxaBinarize,
                    input_file_grp=input_file_grp,
                    output_file_grp=output_file_grp,
                    parameter={"algorithm": algo},
                    **processor_kwargs,
                )
...
def test_pagexml(processor_kwargs, subtests, caplog):
    ...
    for algo in ["Sauvola", "Bernsen", "Wolf", "Gatos", "NICK", "Singh", "Bataineh", "ISauvola", "WAN"]:
        output_file_grp = "OCR-D-GT-PAGE-BIN-%s" % algo.upper()
